...
void AeroDynamics::algorithm(double int_step) {
  ...
  if (liftoff == 1) {
    ...
    cn0 = aerotable.look_up("cn0_vs_mach_alpha", vmach, alppx, 1);
    // normal force coefficient
    cna = cn0;

    // looking up pitching moment coefficients in aeroballistic coord
    clm0 = aerotable.look_up("clm0_vs_mach_alpha", vmach, alppx, 1);
    clmq = aerotable.look_up("clmq_vs_mach", vmach, 1);
    ...
    // ca_on = aerotable.look_up("CA_on_vs_mach_alpha_alt", alppx, alt, vmach,
    // 1);
