...
void GPS_FSW::setup_state_covariance_matrix(double factp, double pclockb,
                                            double pclockf) {
...
void GPS_FSW::setup_error_covariance_matrix(double factq, double qclockb,
                                            double qclockf) {
  this->factq = factq;
...
void GPS_FSW::measure(double int_step) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // SPEED_OF_LIGHT * gps_con->chan[id].rho0.clk(0); SSBIC =
    // (gps_con->chan[id].rho0.pos - SBEEC); double dsbc = norm(SSBIC) -
    // SPEED_OF_LIGHT * gps_con->chan[id].rho0.clk(0);

    // arma::vec3 USSBI;
    // USSBI = SSBIC/norm(SSBIC);

    // // double dvsbc = dot(trans(TEIC) * (gps_con->chan[i].rho0.vel +
    // cross(WEII, gps_con->chan[id].rho0.pos)), (trans(TEIC) *
    // gps_con->chan[id].rho0.pos - SBIIC))/dsbc; double dvsbc =
    // dot(gps_con->chan[i].rho0.vel , SSBIC)/dsbc;
    ...
    double dsb_meas = trans_chan[id].range;  // norm(SSBI) - SPEED_OF_LIGHT *
                                             // gps_con->chan[id].rho0.clk(0);
